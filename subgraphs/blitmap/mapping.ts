...
import {
  AtomicMatch_Call
} from "../../types/Blitmap_OpenSea_Market/OpenSea_Market"
import {
  Transfer
...
export function handleTransfer(e: Transfer): void {
  ...
  if (fromAddress.toHexString() == ZERO_ADDRESS) {
    ...
    let nftId = nfts.getId(bConstants.CONTRACT_ADDRESS, tokenId);
...
export function handleOpenSeaSale(call: AtomicMatch_Call): void {
  ...
  if (isSingleTrade(calldataBuy, calldataSell)) {
    ...
    if (nftAddress.toHexString() == bConstants.CONTRACT_ADDRESS.toHexString()) {
