...
import {
  AtomicMatch_Call
} from "../../types/HashMasks_OpenSea_Market/OpenSea_Market"
import {
  Transfer
} from "../../types/HashMasks_ERC721/HashMasks_ERC721"

/* Hashmasks: all Transfer events
...
export function handleTransfer(e: Transfer): void {
  ...
  if (fromAddress.toHexString() == ZERO_ADDRESS) {
    ...
    let nftId = nfts.getId(hmConstants.CONTRACT_ADDRESS, tokenId);
...
export function handleOpenSeaSale(call: AtomicMatch_Call): void {
  ...
  if (isSingleTrade(calldataBuy, calldataSell)) {
    ...
    if (nftAddress.toHexString() == hmConstants.CONTRACT_ADDRESS.toHexString()) {
